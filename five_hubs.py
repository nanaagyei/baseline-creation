...
for key, value in selected_distances.items():
    ...
    hub_baselines_df = pd.concat([hub_baselines_df, df_temp], ignore_index=True)
...
for i in range(5):
    ...
    site_code_dictionaries.append(code_dict)  # append the dictionary to the list
...
for site_code_dict in site_code_dictionaries:
    map_baseline_df = pd.DataFrame.from_dict(site_code_dict, orient='index', columns=['Site Code'])
...
for value in hub_connection_start_df['start']:
    ...
    hub_connection_start_resulting_df = pd.concat([hub_connection_start_resulting_df, temp_df], ignore_index=True)
...
for value in hub_connection_end_df['end']:
    temp_df = df.loc[df['Site Code'] == value]
    hub_connection_end_resulting_df = pd.concat([hub_connection_end_resulting_df, temp_df], ignore_index=True)
...
def create_map():
    ...
    for j in range(len(filtered_s_hub_map_df)):
        ...
        end_coords = [filtered_s_hub_map_df.iloc[j]['Lat dec'], -1 * abs(filtered_s_hub_map_df.iloc[j]['Long dec'])]
    ...
    for i, row in hub_connection_start_resulting_df.iterrows():
        ...
        end_coords = [hub_connection_end_resulting_df.loc[i, 'Lat dec'], -hub_connection_end_resulting_df.loc[i, 'Long dec']]
